<script lang="ts">
  export const len = 25;

  const cm = Math.floor(len / 10);
  const mm = len % 10;
</script>

<div
  class="box-border absolute w-full"
  style="transform: translateX(0.5px); width: calc(100% + 1px);"
>
  {#each [...Array(cm).keys()] as c}
    <div class="cm" style="width: {1000 / len}%; right:{(1000 * c) / len}%;">
      <div class="translate-x-1">{c}</div>
      <div>
        {#each [...Array(10).keys()] as m}
          <div class="mm " style="width: 10%; right:{10 * m}%;" />
        {/each}
      </div>
    </div>
  {/each}

  <div class="cm" style="width: {1000 / len}%; right:{(1000 * cm) / len}%;">
    <div class="translate-x-1">{cm}</div>
    <div>
      {#each [...Array(mm + 1).keys()] as m}
        <div class="mm " style="width: 10%; right:{10 * m}%;" />
      {/each}
    </div>
  </div>
</div>

<style lang="postcss">
  .cm {
    @apply text-right absolute font-mono h-4 -translate-y-6;
  }
  .mm {
    @apply absolute border-r border-gray-600;
    height: 0.25rem;
  }

  .mm:first-of-type {
    height: 1rem;
  }

  .mm:nth-of-type(6n) {
    height: 0.5rem;
    /* transform: translateY(0.5rem); */
  }
</style>
