...
class Imager:
    ...
    def take_image(self, n_bundles: int) -> FourImages:
        ...
        while not self.y.is_in_position.result():
            logger.warning("Started taking images while y-stage is not in position. Waiting.")
        ...
        with self.optics.open_shutter():
            imgs = self.cams.capture(
                n_bundles,
                start_capture=lambda: self.y.move(end_y_pos, slowly=True, monitor=True),
            ).result()

        logger.info(f"Done taking image.")
        return imgs
