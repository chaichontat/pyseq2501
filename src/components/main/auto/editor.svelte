<script lang="ts">
  ...
  export let flowcell: "A" | "B";
  const idx = flowcell === "B" ? 1 : 0;
</script>
...
<div class="flex items-center">
  <span>
    ...
    <p class="inline-block my-4 text-3xl font-extrabold tracking-tight text-gray-700 dark:text-white">Flowcell {flowcell}</p>
  </span>
  <div class="flex-grow" />
...
{:else}
  <div class="flex items-center justify-center w-full h-[50vh]">
    <div class="inline-flex text-2xl rounded-md shadow-sm" role="group">
      <button type="button" class="px-8 py-6 font-medium rounded-l-lg white-button" on:click={() => ($us.recipes[idx] = { ...recipeDefault })}>
