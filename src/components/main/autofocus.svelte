<script lang="ts">
  ...
  $: if (browser) imgFrame.src = $ls.afimg[step];
...
<div class="space-y-12">
  <div class="flex gap-x-4 max-w-lg mx-auto">
    <div class="flex-grow">
  ...
  <div class="flex items-center">
    <div class="flex flex-col items-center max-w-md mx-auto gap-y-6">
      ...
      <div class="flex gap-x-4">
        <span class="rounded-lg bg-blue-50 py-2 px-4 font-medium">{real_steps[step]}</span>
      ...
      <input type="range" min="0" max="258" bind:value={step} class="range" />
      <Autofocusgraph />
    </div>
  </div>
</div>
